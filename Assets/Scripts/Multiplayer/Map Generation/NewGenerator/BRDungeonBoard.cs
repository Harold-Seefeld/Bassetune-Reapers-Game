...
public class BRDungeonBoard {
    private XGrid _grid;
    private List<IShape> _roomsAndCorridors;

    public BRDungeonBoard(int rows, int columns) 
        : this(new XGrid(rows, columns)) {
    } 
        
    public BRDungeonBoard(XGrid mapGrid) {
        _grid = mapGrid;
    ...
    public void add(XRoom aRoom) {
        if (_roomsAndCorridors.Count != 0) {
            XCorridor corr = (XCorridor)_roomsAndCorridors[_roomsAndCorridors.Count - 1];
    ...
    public void add(XCorridor corr) {
        if (_roomsAndCorridors.Count != 0) {
            XRoom room = (XRoom)_roomsAndCorridors[_roomsAndCorridors.Count - 1];
    ...
    public void removeLast() {
        ...
        if (last is XRoom) {
            XRoom room = (XRoom)last;
            room.setCorridorIncoming(null);
            if (beforeLast != null) {
                XCorridor corr = (XCorridor)beforeLast;
                corr.setDestinationRoom(null);
            }
        } else {
            XCorridor corr = (XCorridor)last;
            corr.setSourceRoom(null);
            if (beforeLast != null) {
                XRoom room = (XRoom)_roomsAndCorridors[_roomsAndCorridors.Count - 2];
