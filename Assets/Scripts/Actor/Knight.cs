...
{
    // animation parmeters
    private const string AniMoveSpeedName = "MoveSpeed";
    private const string AniAttackedName = "Attacked";
    ...
    private void Update()
    {
        ...
        if (currentAbility != -1) setTarget(ref target, ref targetPos);
    }
    ...
    protected override void OnCastLegacy()
    {
        ...
        if (Input.GetAxis("Attack1") > 0)
        {
            if (abilities.Length > 0 && abilities[0]) currentAbility = 0;
        }
        else if (Input.GetAxis("Attack2") > 0)
        {
            if (abilities.Length > 1 && abilities[1]) currentAbility = 1;
        }
        else if (Input.GetAxis("Attack3") > 0)
        {
            if (abilities.Length > 2 && abilities[2]) currentAbility = 2;
        }
        else if (Input.GetAxis("Attack4") > 0)
        {
            if (abilities.Length > 3 && abilities[3]) currentAbility = 3;
        }
        else if (Input.GetAxis("Attack5") > 0)
        {
            if (abilities.Length > 4 && abilities[4]) currentAbility = 4;
        }
        else if (Input.GetAxis("Attack6") > 0)
        {
            if (abilities.Length > 5 && abilities[5]) currentAbility = 5;
        }
        else if (Input.GetAxis("Attack7") > 0)
        {
            if (abilities.Length > 6 && abilities[6]) currentAbility = 6;
        }
        else if (Input.GetAxis("Attack8") > 0)
        {
            if (abilities.Length > 7 && abilities[7]) currentAbility = 7;
        }
    }
